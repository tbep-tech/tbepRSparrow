#####################################################################################################
# estimateFeval:  SPARROW Function to Optimize
...
#################################################
estimateFeval <- function(beta0,batch_mode,ErrorOccured) {

  # INPUT DATA (objects must be stored in global environment):
...
#reservoir_decay_specification <- "(1 / (1 + data[,jresvar[i]] * beta1[,jbresvar[i]]))"
  ...
  data <- DataMatrixEstimate.list$data
  beta <- DataMatrixEstimate.list$beta
  ...
  for(i in 1:length(DataMatrixEstimate.list$data.index.list)){
    tempobj=DataMatrixEstimate.list$data.index.list[[i]]
    eval(parse(text=paste(names(DataMatrixEstimate.list$data.index.list)[[i]],"= tempobj")))
  }
  ...
  #   compute and accumulate incremental RCHLD
    ...
    if (class(tryIt)=="try-error"){#if an error occured
      if(ErrorOccured=="no"){
        errorOccurred("estimateFeval.R",batch_mode)
      }
    }else{#if no error
    return(e) 
    }#end if error
