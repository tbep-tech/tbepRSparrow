...
# Obtain matrices for predictions for computing CIs

# The total load is computed as the ratio of the squared NLLS prediction (with mean error correction removed) to the
#   product of the bootstrap prediction with a randomly selected NLLS residual error per equation 1.150

 predictBootCI <- function(predict.list,bootcorrection,bEstimate,estimate.list,ConcFactor,bootcorrectionR,DataMatrix.list,SelParmValues,
...
#################################################
  ...
  # Output load predictions
    ...
    # total load, computed as the ratio of the squared NLLS prediction (with mean error correction removed) to the
    #   product of the bootstrap prediction with a randomly selected NLLS residual error per equation 1.150
    predmatrix[,2] <- (predict.list$predmatrix[,2]/bootcorrection)^2 / (pload_total * bootcorrectionR)
    for (i in 1:length(srclist_total)){
      predmatrix[,2+i] <- (predict.list$predmatrix[,2+i]/bootcorrection)^2 / (eval(parse(text=srclist_total[i])) * bootcorrectionR)
    ...
    predmatrix[,(4+length(srclist_total)+length(srclist_mtotal))] <- (predict.list$predmatrix[,(4+length(srclist_total)+length(srclist_mtotal))]/bootcorrection)^2 / (pload_nd_total * bootcorrectionR)
    for (i in 1:length(srclist_nd_total)){
      predmatrix[,(4+length(srclist_total)+length(srclist_mtotal)+i)] <- (predict.list$predmatrix[,(4+length(srclist_total)+length(srclist_mtotal)+i)]/bootcorrection)^2 / (eval(parse(text=srclist_nd_total[i])) * bootcorrectionR)
    }
    # incremental load
    predmatrix[,(5+length(srclist_total)+length(srclist_mtotal)+length(srclist_nd_total))] <- (predict.list$predmatrix[,(5+length(srclist_total)+length(srclist_mtotal)+length(srclist_nd_total))]/bootcorrection)^2 / (pload_inc * bootcorrectionR)
    for (i in 1:length(srclist_inc)){
      predmatrix[,(5+length(srclist_total)+length(srclist_mtotal)+length(srclist_nd_total)+i)] <- (predict.list$predmatrix[,(5+length(srclist_total)+length(srclist_mtotal)+length(srclist_nd_total)+i)]/bootcorrection)^2 / (eval(parse(text=srclist_inc[i])) * bootcorrectionR)
...
# Output yield predictions
    ...
    predictBoots.list <- named.list(oparmlist,loadunits,predmatrix,oyieldlist,yieldunits,yldmatrix,predict.source.list)
#  assign("predict.list",predict.list, envir=.GlobalEnv)
     ...
     if (class(tryIt)=="try-error"){#if an error occured
       if(ErrorOccured=="no"){
         errorOccurred("predictBootCI.R",batch_mode)
       }
     }else{#if no error
