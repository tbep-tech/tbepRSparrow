...
#'@description Checks for excessively large integer values for the variables
#'WATERID, TNODE, FNODE, HYDSEQ to avoid array storage problems in R and fortran subroutines
...
checkData1NavigationVars <- function(data1,if_reverse_hydseq,batch_mode,ErrorOccured) {
  if (ErrorOccured=="no"){
    tryIt<-try({ 
