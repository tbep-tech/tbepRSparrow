#'@title calcTermflag
#'@description calculates termflag
#'Uses subroutines: errorOccurred. 
#'@param data1 dataset to use for termflag calculation
...
#'@return data.frame with waterid and termflag


calcTermflag<-function(data1, batch_mode,ErrorOccured){
  if (ErrorOccured=="no"){
    tryIt<-try({
    ...
    if (class(tryIt)=="try-error"){#if an error occured
      if(ErrorOccured=="no"){
        errorOccurred("calcTermflag.R",batch_mode)
      }
    }else{#if no error
      return(outdata)
    }#end if error
