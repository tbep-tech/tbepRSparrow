#'@title interactivePredictionSettings
#'@description Generates list of prediction map setting checks and error messages for Rshiny
...
interactivePredictionSettings<-function(input){
  interactiveSetting<-list()
  if (input$batch=="Batch"){
    checks<-names(input)[which(regexpr("Check",names(input))>0 & names(input)!="outCheck")]
    mapVar<-character(0)
    for (c in checks){
      mapVar<-c(mapVar,paste0("length(input$",c,"!=0)"))
    }
    mapVar<-paste(mapVar, collapse = "|",sep=" ")
    
setMessage<-"Please select Prediction Metrics"
setExample<-setMessage
  }else{
    mapVar<-c("!is.na(input$mapCategory)",
    "input$mapCategory!=''",
    
    "!is.na(input$var)",
    "input$var!=''")
    setMessage<-c("Please select a Prediction Type",
                  "Please select a Prediction Type",
                  
                  "Please select a Prediction Metric",
                  "Please select a Prediction Metric")
    setExample<-setMessage
  }
