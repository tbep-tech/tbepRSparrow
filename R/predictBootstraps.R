...
# predictBootstraps.R
#
# Executes "if_boot_predict" yes option

##########################################################
# Calculate and output error-corrected predictions for monitoring-nonadjusted loads
#   Note: No error-correction for monitoring-adjusted loads and delivery fraction
   
predictBootstraps <- function(iseed,biters,estimate.list,predict.list,BootResults,confInterval,
                              DataMatrix.list,SelParmValues,
                              reach_decay_specification,reservoir_decay_specification,subdata,ConcFactor,yieldFactor,
                              path_results,file_sum,batch_mode,ErrorOccured) {
  if (ErrorOccured=="no"){
    tryIt<-try({ 
  ...
  for(i in 1:length(predict.list$predict.source.list)){
    tempobj=predict.list$predict.source.list[[i]]
    eval(parse(text=paste(names(predict.list$predict.source.list)[[i]],"= tempobj")))
  }
  ...
  for (i in 1:length(datalstCheck)) {
   ...
   numsites <- sum(ifelse(DataMatrix.list$data[,10] > 0,1,0))  # jdepvar site load index
   ...
   for (iter in 1:biters) {
     ...
     bootcorrectionR <- 1
     predBoots.list <- predictBoot(oEstimate,estimate.list,ConcFactor,bootcorrectionR,DataMatrix.list,SelParmValues,
...
# Output bootstrap yield predictions
   ...
   assign("predictBoots.list",predictBoots.list,envir = .GlobalEnv)
    ...
    if (class(tryIt)=="try-error"){#if an error occured
      if(ErrorOccured=="no"){
        errorOccurred("predictBootstraps.R",batch_mode)
      }
    }else{#if no error
