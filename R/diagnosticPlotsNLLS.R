...
#'@description Creates diagnostic plots output to ~/estimate/(run_id)_diagnostic_plots.pdf
#'Uses subroutines: checkBinaryMaps, mapSiteAttributes, diagnosticMaps, errorOccurred.
...
#
# diagnosticPlotsNLLS.R
#
#############################################################################
# Create diagnostic plots

diagnosticPlotsNLLS<- function(path_results,file_sum,classvar,class_landuse,sitedata.demtarea.class,
                               sitedata,sitedata.landuse,estimate.list,mapping.input.list,Csites.weights.list,
                               data_names,add_vars,batch_mode,ErrorOccured) {
  ...
  filename <- paste(path_results,"/estimate/",file_sum,"_diagnostic_plots.pdf",sep="")
  ...
  if (!is.na(map_siteAttributes.list) & existGeoLines==TRUE){
    strExplanation<-paste("
     ...
     strExplanation<-paste("
      ...
      title(main=paste(file_sum,"_diagnostic_plots.pdf Document Contents",sep=""))
  ...
  if(existGeoLines==TRUE) { 
    ...
    for (s in 1:length(map_siteAttributes.list)){
      ...
      mapSiteAttributes(#Rshiny
                        ...
                        #regular
                        names(siteAttr),mapdata,GeoLines,mapping.input.list,titleAttr,unitAttr,batch_mode,ErrorOccured)
      }else{
 ...
 #8.8.17 if(!is.na(LineShapeGeo)) {
  ...
  diagnosticMaps("Resids",mapdata,GeoLines,lat_limit,lon_limit,
                 c("threshold","all"),"Model Estimation Log Residuals",mapping.input.list,batch_mode,ErrorOccured)
   ##########################  
    ...
    diagnosticMaps("pResids",pmapdata,GeoLines,lat_limit,lon_limit,
                   c("threshold","all"),"Model Simulation Log Residuals",mapping.input.list,batch_mode,ErrorOccured)
      
    
    #if (if_estimate_simulation=="no"){
    #Map Standardized Residuals
    #          smapdata <- data.frame(xlat,xlon,standardResids,pstudentResids)
    smapdata <- data.frame(xlat,xlon,standardResids)
    diagnosticMaps("standardResids",smapdata,GeoLines,lat_limit,lon_limit,
                   c("threshold","all"),"Model Estimation Standardized Residuals",mapping.input.list,batch_mode,ErrorOccured)
    #}
    
  ##########################
  # Map Ratios observed to predicted 
      diagnosticMaps("ratio.obs.pred",mapdata,GeoLines,lat_limit,lon_limit,
                     c("threshold","all"),"Model Estimation Obs/Pred Ratio",mapping.input.list,batch_mode,ErrorOccured)
  
  ##########################
  # Map Ratios observed to predicted for no monitoring adjustment
        diagnosticMaps("pratio.obs.pred",pmapdata,GeoLines,lat_limit,lon_limit,
                       c("threshold","all"),"Model Simulation Obs/Pred Ratio",mapping.input.list,batch_mode,ErrorOccured)
  
     ########################
            #Map of predicted studentized residuals
            #diagnosticMaps("pstudentResids",smapdata,GeoLines,lat_limit,lon_limit,residual_map_breakpoints,
#                           c("threshold","all"),"Predicted Studentized Residuals",batch_mode,ErrorOccured)
  ...
  #coordinates(siteAttrshape)<- ~xlon+xlat
    ...
    if (!dir.exists(paste(path_results,"maps/ESRI_ShapeFiles/siteAttributes/",sep=""))){
      dir.create(paste(path_results,"maps/ESRI_ShapeFiles/siteAttributes/",sep=""),showWarnings = FALSE)
    }
    
    maptools::writeSpatialShape(siteAttrshape,paste(path_results,"maps/ESRI_ShapeFiles/siteAttributes/siteAttrshape",sep=""))
    cat(showWKT(proj4string(siteAttrshape)),file=paste(path_results,"/maps/ESRI_ShapeFiles/siteAttributes/siteAttrshape.prj",sep="")) 
  }
  ...
  if (outputERSImaps[3]=="yes"){
    ...
    tiareakm2<-Csites.weights.list$tiareakm2
    weight<-Csites.weights.list$weight
    origWaterid<-sitedata$waterid_for_RSPARROW_mapping
    
    dd <- data.frame(sitedata,origWaterid,Obs,predict,Obsyield,predictYield,Resids,standardResids,leverage,boot_resid,weight,tiareakm2,pResids,ratio.obs.pred,pratio.obs.pred,xlat, xlon)
    keeps <- c("waterid","origWaterid","demtarea","rchname","station_id","station_name","staid",classvar[1],"Obs",
               "predict","Obsyield","predictYield","Resids","standardResids","leverage","boot_resid","weight","tiareakm2","pResids","ratio.obs.pred","pratio.obs.pred","xlat","xlon")
    residShape <- dd[keeps]
    ...
    maptools::writeSpatialShape(residShape,paste(path_results,"maps/ESRI_ShapeFiles/residuals/residShape",sep=""))
    cat(showWKT(proj4string(residShape)),file=paste(path_results,"/maps/ESRI_ShapeFiles/residuals/residShape.prj",sep="")) 
    ...
    if (class(tryIt)=="try-error"){#if an error occured
      if(ErrorOccured=="no"){
        errorOccurred("diagnosticPlotsNLLS.R",batch_mode)
      }
    }else{#if no error
