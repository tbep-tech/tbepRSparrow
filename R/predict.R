...
# Calculates predictions of load
 ...
 predict <- function(estimate.list,ConcFactor,yieldFactor,ConcUnits,yieldUnits,bootcorrection,DataMatrix.list,SelParmValues,
...
#################################################
  ...
  # Output load predictions
    ...
    predmatrix[,(3+length(srclist_total))] <- mpload_total * bootcorrection
    for (i in 1:length(srclist_mtotal)){
      predmatrix[,(3+length(srclist_total)+i)] <- eval(parse(text=srclist_mtotal[i])) * bootcorrection
    } 
...
# Output yield predictions
  ...
  assign("predict.list",predict.list, envir=.GlobalEnv)
     ...
     if (class(tryIt)=="try-error"){#if an error occured
       if(ErrorOccured=="no"){
         errorOccurred("predict.R",batch_mode)
       }
     }else{#if no error
 return(predict.list)
     }#end if error
